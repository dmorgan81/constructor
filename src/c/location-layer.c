...
static void settings_handler(void *this) {
    log_func();
    Data *data = fctx_layer_get_data(this);
    FPoint offset = FPointI(enamel_get_LOCATION_X(), enamel_get_LOCATION_Y());
    uint32_t rotation = DEG_TO_TRIGANGLE(enamel_get_LOCATION_ROTATION());
    GTextAlignment alignment = atoi(enamel_get_LOCATION_ALIGNMENT());

    fctx_rect_layer_set_fill_color(data->rect_layer, enamel_get_LOCATION_RECT_FILL_COLOR());
    FSize size = FSizeI(enamel_get_LOCATION_RECT_SIZE_W(), enamel_get_LOCATION_RECT_SIZE_H());
    ...
    fctx_rect_layer_set_border_color(data->rect_layer, enamel_get_LOCATION_RECT_BORDER_COLOR());
    fctx_rect_layer_set_border_width(data->rect_layer, enamel_get_LOCATION_RECT_BORDER_WIDTH());
    fctx_rect_layer_set_alignment(data->rect_layer, alignment);

    fctx_text_layer_set_alignment(data->text_layer, alignment);
    fctx_text_layer_set_em_height(data->text_layer, enamel_get_LOCATION_FONT_SIZE());
    fctx_text_layer_set_fill_color(data->text_layer, enamel_get_LOCATION_COLOR());
